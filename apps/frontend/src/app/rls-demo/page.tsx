...
import { RlsExample } from "../../../../../packages/db/src/types";
...
export default function RlsDemoPage() {
  ...
  const [currentSseExample, setCurrentSseExample] = useState<RlsExample | null>(null);
  ...
  const { data: rlsExamples, isLoading, error } = useQuery({
    ...trpc.rlsDemo.listForUser.queryOptions(),
  ...
  const createMutation = useMutation({
    ...trpc.rlsDemo.create.mutationOptions(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: trpc.rlsDemo.listForUser.queryKey() });
  ...
  const deleteMutation = useMutation({
    ...trpc.rlsDemo.delete.mutationOptions(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: trpc.rlsDemo.listForUser.queryKey() });
  ...
  const updateMutation = useMutation({
    ...trpc.rlsDemo.update.mutationOptions(),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: trpc.rlsDemo.listForUser.queryKey() });
      if (data?.publicToken) {
        const publicLink = `${window.location.origin}/public/rls-example/${data.publicToken}`;
  ...
  useSubscription(
    {
      ...trpc.rlsDemo.streamExamples.subscriptionOptions(
        ...
        <div className="flex items-center gap-2 h-full">
          {!params.row.publicToken ? (
            ...
            <Button 
              ...
              onClick={(e) => {
                e.stopPropagation();
                const publicLink = `${window.location.origin}/public/rls-example/${params.row.publicToken}`;
  ...
  if (error) return (
    <Layout>
      <div>Error loading RLS examples</div>
    </Layout>
  );
  ...
  return (
    <Layout>
      <div className="p-16">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-2xl font-bold">RLS Demo</h1>
            <p className="text-muted-foreground">
              Demonstrates Row Level Security with user-owned content and public sharing
            </p>
          </div>
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Create New RLS Example</DialogTitle>
            </DialogHeader>
        ...
        <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Delete RLS Example</DialogTitle>
            </DialogHeader>
            <div className="py-4">
