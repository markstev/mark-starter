...
/**
 * Set a feature flag for a specific organization (admin function)
...
export async function setOrganizationFeatureFlag(
  ...
  try {
    ...
    console.error(`Error setting feature flag ${flagName} for organization ${organizationId}:`, error);
    return false;
  }
}
...
export async function getFlagStateForOrganization(
  organizationId: string, 
  userId?: string | null
): Promise<Array<{
  name: string;
  ...
  const result = await getFlagState(organizationId, userId);
  return result.map(flag => ({
    name: flag.name,
    description: flag.description,
    defaultState: flag.defaultState,
    organizationOverride: flag.organizationOverride,
    userOverride: flag.userOverride,
    effectiveState: flag.effectiveState
  }));
}
